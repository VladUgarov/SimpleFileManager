...
export async function createFile(fileName) {
  ...
  return new Promise((resolve, reject) => {
    fs.writeFile(path.resolve('files', fileName), '', (error) => {
      if (error) {
...
export async function deleteFile(fileName) {
  fileName += '.txt';
  return new Promise((resolve, reject) => {
    fs.unlink(path.resolve('files', fileName), (error) => {
      if (error) {
...
export async function readFile(fileName) {
  fileName += '.txt';
  return new Promise((resolve, reject) => {
    fs.readFile(path.resolve('files', fileName), 'utf-8', (error, data) => {
      if (error) {
...
export async function updateFile(fileName, fileContent) {
  fileName += '.txt';
  return new Promise((resolve, reject) => {
    fs.writeFile(path.resolve('files', fileName), fileContent, (error) => {
      if (error) {
